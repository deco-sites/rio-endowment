...
const SliderInfoCard = ({ title, infoCards }: Props) => {
    ...
    return (
        <main id={id} class="px-4 mb-24 max-w-7xl mx-auto">
            <div
                class="mt-[76px] mb-8 [&_span]:leading-none [&_span]:tracking-tighter"
                dangerouslySetInnerHTML={{ __html: title }}
            />
            {device !== "desktop" ? (
                <div class="flex flex-col gap-14">
                    {infoCards.map(({ imageMobile, text }) => (
                        <div class="bg-gray-500 rounded-[40px] overflow-hidden">
                            <Image
                                class="sm:w-full"
                                alt={imageMobile.alt}
                ...
                <>
                    <Slider class="carousel carousel-center gap-14" rootId={id}>
                        {infoCards.map(({ imageDesktop, text }, index) => (
                            <Slider.Item class="carousel-item w-full max-w-[936px]" index={index}>
