...
const GridBanner = ({ lines, sectionDescription, sectionTitle }: Props) => {
    ...
    return (<>
            <section class="block mb-32 px-4 lg:px-0">
                <h1 class={clx("scroll-animate", "font-bold text-center text-blue-100 text-2xl/7", "lg:text-3xl/relaxed")}>
                    {sectionTitle}
                </h1>
                <p class="scroll-animate text-black-100 text-center text-sm/5 mb-14 lg:text-xl/loose">{sectionDescription}</p>
        
                <div class="flex flex-col gap-5">
                    {lines.map(({ images, verticalAlign }) => (<div class="scroll-animate flex gap-x-2 gap-y-6 justify-center flex-wrap lg:gap-5" style={{ alignItems: verticalAlign }}>
                            {images.map(({ alt, height, image, width, link }) => (<a href={link} alt={alt}>
