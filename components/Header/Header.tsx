    ...
    globalThis?.addEventListener("scroll", () => {
        if (globalThis.scrollY <= 150) {
            document.getElementById("header-container")!.style.backgroundColor =
                "transparent";
            document.getElementById("header-container")!.style.boxShadow =
                "0px 0px 0px 0px transparent";
        } else {
            document.getElementById("header-container")!.style.backgroundColor =
                "#fff";
            document.getElementById("header-container")!.style.boxShadow =
                "0px 5px 20px 0px #1C1C1E14";
        }
    });
};
        ...
        <>
            {device === "desktop" && (
                <header
                    id="header-container"
                    class={clx(
                        "fixed top-0 left-1/2 w-full z-20 -translate-x-1/2",
                        "flex justify-between items-center py-2.5 px-14",
                        "bg-transparent transition-colors max-w-[1920px]"
                    )}
                            ...
                            <div class="flex flex-col gap-12 bg-white w-full min-h-[90%] rounded-[40px] p-8">
                                ...
                                <ul class="flex flex-col gap-6">
                                    {navItems.map(({ navText, link }) => (
                                        <li>
                                            <a
                                                class="font-medium text-2xl text-blue-100"
                                                href={link}
                                            >
                                                {navText}
                                            </a>
